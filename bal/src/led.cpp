...
LedStatus::LedStatus(const char* led_name, const oshal::Gpio& gpio,
                     bool is_active_low)
    : led_name_(led_name),
      gpio_(gpio),
      is_active_low_(is_active_low),
      is_initialized_(false) {}

const char* LedStatus::name() const { return led_name_; }

bool LedStatus::is_ready() const { return gpio_.is_ready(); }
...
int initialize_leds() { return internal::status_led_backend().initialize(); }

Led& status_led() { return internal::status_led_backend(); }
