...
/// @brief Optional capability interface for PWM outputs that support pulse
/// sequencing.
...
class PwmSequenceOutput {
 public:
  ...
  /// @brief Play a sequence of pulse widths while keeping the configured
  /// period.
  ...
  /// @return STATUS_OK on success, or a negative project-defined status code on
  ///     failure.
  virtual int play_pulse_sequence(const std::uint32_t* pulse_ns_sequence,
                                  std::size_t pulse_count,
                                  std::uint32_t repeat_count) = 0;

  /// @brief Stop an active pulse sequence and keep the current PWM
  /// configuration.
