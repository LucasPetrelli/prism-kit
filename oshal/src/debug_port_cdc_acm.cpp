...
ZephyrCdcAcmDebugPort::ZephyrCdcAcmDebugPort(const char *port_name, const device *device)
	: name_(port_name), device_(device), pending_buffer_(nullptr), pending_length_(0U),
	  pending_status_(STATUS_OK), tx_irq_callback_bound_(false)
{
	/*
	 * The CDC ACM backend keeps its own small synchronization model because the
	 * USB class driver does not expose a simple blocking uart_tx() API. Instead we
	 * queue bytes into the CDC ACM UART FIFO whenever it reports TX-ready and let
	 * the caller sleep until the current request is drained.
	 */
	k_mutex_init(&write_mutex_);
	k_sem_init(&write_complete_, 0U, 1U);

	/*
	 * If this callback registration fails there is no useful FIFO-driven transport
	 * path left for this backend, so later writes will fail fast with
	 * STATUS_ERR_BACKEND instead of hanging while waiting for a completion event
	 * that can never be signaled.
	 */
	tx_irq_callback_bound_ = uart_irq_callback_user_data_set(
		device_, &ZephyrCdcAcmDebugPort::tx_irq_callback, this) == 0;
}

const char *ZephyrCdcAcmDebugPort::name() const
{
	return name_;
}
...
int ZephyrCdcAcmDebugPort::write(const char *buffer, std::size_t length) const
{
	...
	/*
	 * Serialize callers so the backend only ever owns one pending write request at
	 * a time. That keeps the state machine simple: one caller publishes one byte
	 * span, the callback drains it, and the caller wakes when that span reaches
	...
	{
		/*
		 * The callback path advances these fields as it feeds bytes into the CDC ACM
		 * TX FIFO, so publish them under the spinlock before enabling TX IRQ.
		 */
	 ...
	 * serviced from the USB workqueue. Enabling TX IRQ causes the driver to invoke
	 * the callback whenever more FIFO space is available. That gives this backend
	 * a transport-aware blocking write path: the caller sleeps on a semaphore
	 * while the workqueue and callback path push chunks through the CDC ACM FIFO.
	 *
	...
	/*
	 * Completion means "the whole pending request has been copied into the CDC ACM
	 * FIFO path", not necessarily "the host application has consumed every byte".
	 * That matches the contract of most serial write APIs and keeps the backend
	 * from pretending it can observe end-to-end host consumption.
	 */
	...
	{
		/*
		 * Tear down the published request state before releasing the writer mutex so
		 * the next caller always starts from a clean slate.
		 */
		const k_spinlock_key_t key = k_spin_lock(&state_lock_);
...
int ZephyrCdcAcmDebugPort::vprintf(const char *format, std::va_list args) const
{
	if (format == nullptr) {
	 ...
	 * transport-specific write path a chance to hand larger spans to uart_fifo_fill
	 * and makes the whole path meaningfully different from the generic poll_out()
	 * fallback.
	 */
...
void ZephyrCdcAcmDebugPort::tx_irq_callback(const device *device, void *user_data)
{
	ARG_UNUSED(device);
	/*
	 * Zephyr's interrupt-driven UART callback API delivers user_data, which lets
	 * the backend bounce directly back into its instance method without relying on
	 * any file-global singleton state.
	 */
...
int ZephyrCdcAcmDebugPort::format_char_out(int c, void *context)
{
	...
	/*
	 * Accumulate a short run of formatted bytes locally. Once the scratch buffer is
	 * full, flush it through the CDC ACM transport-specific write path and keep
	 * formatting. This keeps the formatter simple while still avoiding per-byte
	 * transport calls.
	 */
...
int ZephyrCdcAcmDebugPort::flush_format_buffer(FormatBuffer *buffer) const
{
	...
	/*
	 * Reuse write() so both raw writes and formatted writes share exactly the same
	 * FIFO-draining, semaphore-blocking transport logic. That keeps the CDC ACM
	 * behavior consistent no matter how bytes enter the backend.
	 */
...
void ZephyrCdcAcmDebugPort::service_tx_fifo() const
{
	 ...
	 * same: copy as much of the pending byte span into the FIFO as the driver will
	 * currently accept, then either wait for another callback or complete the
	 * write.
	 */
	while (true) {
		...
		if (completed) {
			/* Entire request is handed off into the FIFO path; wake the blocked writer. */
			uart_irq_tx_disable(device_);
			k_sem_give(&write_complete_);
			return;
		}
