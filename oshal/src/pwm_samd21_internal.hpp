...
protected:
	...
	/// @brief Stop any subclass-managed transient activity before reconfiguration.
	/// @return STATUS_OK on success, or a negative status code on failure.
...
class Samd21DmaPwmOutput final : public Samd21PwmOutput, public PwmSequenceOutput {
public:
	...
	Samd21DmaPwmOutput(const char *pwm_name, Tcc *regs, std::uint8_t channel_index,
		std::uint8_t port_group_index, std::uint8_t port_pin_index, std::uint8_t pin_mux_value,
	...
	/// @return STATUS_OK on success, or a negative status code on failure.
	int play_pulse_sequence(const std::uint32_t *pulse_ns_sequence, std::size_t pulse_count,
...
protected:
	/// @brief Stop active DMA activity before base PWM reconfiguration or disable.
	/// @return STATUS_OK.
...
private:
	...
	static int resolve_dma_trigger_source(const Tcc *regs, std::uint8_t channel_index,
	...
	/// @return STATUS_OK on success, or a negative status code on failure.
	int build_dma_sequence(const std::uint32_t *pulse_ns_sequence, std::size_t pulse_count);
