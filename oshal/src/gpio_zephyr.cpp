...
const char* ZephyrGpio::name() const { return name_; }

bool ZephyrGpio::is_ready() const { return gpio_is_ready_dt(&spec_); }
...
int ZephyrGpio::configure_output(bool initial_high) const {
  ...
  /* Configuration owns the first output level so callers do not need a second
   * write. */
...
int ZephyrGpio::set(bool high) const {
  ...
  /* Keep physical writes normalized to OSHAL status codes before they leave the
   * backend. */
  return status_from_backend_result(
    gpio_pin_set(spec_.port, spec_.pin, high ? 1 : 0));
}
