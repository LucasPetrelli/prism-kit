...
/* Start each snapshot from a clean zeroed state so unsupported fields stay explicit. */
 ...
 */
int copy_runtime_sample(oshal::TaskRuntimeSample *out_sample, k_tid_t thread_id)
{
 ...
 */
bool try_get_current_stack_used_bytes(k_tid_t thread_id, std::size_t *out_bytes)
{
	...
	uintptr_t current_stack_pointer = static_cast<uintptr_t>(thread->callee_saved.psp);
	...
	*out_bytes = static_cast<std::size_t>(initial_stack_pointer - current_stack_pointer);
	return true;
#else
	ARG_UNUSED(thread_id);
	return false;
...
{
	k_spinlock_key_t key = k_spin_lock(&slot.lock);
	const bool still_matches = slot.allocated && (slot.generation == snapshot.generation);
	if (!still_matches) {
		k_spin_unlock(&slot.lock, key);
	...
	if (update_previous_sample) {
		if ((out_previous_sample == nullptr) || (out_have_previous_sample == nullptr)) {
			k_spin_unlock(&slot.lock, key);
			return STATUS_ERR_INVALID_ARGUMENT;
		}
	...
	bool update_previous_sample)
{
	...
	for (std::size_t attempt = 0U; attempt < kSnapshotRetryLimit; ++attempt) {
		...
		out_info->configured_stack_size_bytes = snapshot.configured_stack_size_bytes;
		...
		if (k_thread_stack_space_get(snapshot.thread_id, &unused_stack_bytes) != 0) {
			return STATUS_ERR_BACKEND;
		}
		...
		if (try_get_current_stack_used_bytes(snapshot.thread_id, &current_stack_used_bytes)) {
			out_info->current_stack_used_bytes = current_stack_used_bytes;
		}

		const int sample_ret = copy_runtime_sample(&out_info->runtime_sample, snapshot.thread_id);
		if (sample_ret < 0) {
			return sample_ret;
		}
		...
		const int finalize_ret = finalize_runtime_snapshot(
			slot, snapshot, *out_info, update_previous_sample, &previous_sample, &have_previous_sample);
		...
		if (update_previous_sample && have_previous_sample) {
			...
			if (calculate_runtime_load(&runtime_load, previous_sample, out_info->runtime_sample) ==
			    STATUS_OK) {
...
int TaskHandle::runtime_info(TaskRuntimeInfo *out_info) const
{
	...
	internal::TaskSlot *const slot = internal::find_task_slot(slot_index_, generation_);
