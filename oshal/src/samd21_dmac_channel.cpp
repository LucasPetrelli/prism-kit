...
/* Constructor only captures immutable channel identity and resets software state. */
...
int Samd21DmacChannel::initialize_controller()
{
	...
	DMAC->WRBADDR.reg = reinterpret_cast<std::uint32_t>(&g_dmac_writeback_descriptors[0]);
...
int Samd21DmacChannel::configure_word_stream(const volatile std::uint32_t *destination_register,
	const std::uint32_t *source_words, std::size_t word_count, std::uint8_t trigger_source,
	bool loop_forever, CompletionCallback callback, void *callback_context)
{
	int ret;
	DmacDescriptor *descriptor;

	if ((destination_register == nullptr) || (source_words == nullptr) || (word_count == 0U) ||
		(word_count > 0xFFFFU) || (channel_index_ >= DMAC_CH_NUM)) {
	...
	descriptor->BTCTRL.reg = DMAC_BTCTRL_VALID | DMAC_BTCTRL_BEATSIZE_WORD | DMAC_BTCTRL_SRCINC;
	descriptor->BTCNT.reg = static_cast<std::uint16_t>(word_count);
	descriptor->SRCADDR.reg =
		reinterpret_cast<std::uint32_t>(source_words + static_cast<std::ptrdiff_t>(word_count));
	descriptor->DSTADDR.reg = reinterpret_cast<std::uint32_t>(destination_register);
	descriptor->DESCADDR.reg = loop_forever ? reinterpret_cast<std::uint32_t>(descriptor) : 0U;
...
bool Samd21DmacChannel::is_running() const
{
	return configured_ && running_;
}
...
extern "C" void DMAC_Handler(void)
{
	...
	while (DMAC->INTSTATUS.reg != 0U) {
		...
		if ((channel_id < DMAC_CH_NUM) && (g_dmac_channel_owners[channel_id] != nullptr)) {
