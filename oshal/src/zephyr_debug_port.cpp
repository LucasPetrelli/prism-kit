...
ZephyrDebugPort::ZephyrDebugPort(const char* port_name, const device* device)
    : name_(port_name),
      device_(device),
      blocking_poll_mode_checked_(false),
      blocking_poll_mode_enabled_(false) {}

const char* ZephyrDebugPort::name() const { return name_; }
...
int ZephyrDebugPort::enable_blocking_poll_mode() const {
   ...
   * up.
   *
   * This is intentionally cached after the first successful or failed attempt
   * so the generic backend does not reconfigure the console device on every
   * write.
   */
...
int ZephyrDebugPort::vprintf(const char* format, std::va_list args) const {
   ...
   * consequence of that C callback typedef, not an attempt to bypass type
   * safety elsewhere in the design.
   */
  const int format_result =
    z_cbvprintf_impl(reinterpret_cast<cbprintf_cb>(debug_port_char_out),
