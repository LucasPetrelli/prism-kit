...
int print_task_runtime_info(const oshal::TaskRuntimeInfo& task_info) {
  ...
  if (task_info.cpu_runtime_percent != oshal::kTaskRuntimePercentUnavailable) {
    static_cast<void>(
      std::snprintf(cpu_runtime_text, sizeof(cpu_runtime_text), "%u%%",
  ...
  if (task_info.current_stack_used_bytes != oshal::kTaskStackUsageUnavailable) {
    static_cast<void>(std::snprintf(current_stack_text,
                                    sizeof(current_stack_text), "%zu",
                                    task_info.current_stack_used_bytes));
  }

  return oshal::debug_port.printf(
    "Task %s runtime: cpu=%s stack cfg=%zu tracked=%zu unused=%zu high=%zu "
    "current=%s\n",
    task_info.name, cpu_runtime_text, task_info.configured_stack_size_bytes,
    task_info.tracked_stack_size_bytes, task_info.unused_stack_bytes,
    task_info.high_water_stack_used_bytes, current_stack_text);
}
}  // namespace
