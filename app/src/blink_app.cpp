...
constexpr std::uint32_t kRuntimeReportStepInterval =
  kRuntimeReportPeriodMs / kDemoStepPeriodMs;
...
constexpr std::uint32_t pulse_width_for_percent(
  std::uint8_t duty_cycle_percent) {
  return (kPwmPeriodNs * static_cast<std::uint32_t>(duty_cycle_percent)) / 100U;
}

constexpr std::uint32_t kPulseSequenceNs[] = {
  pulse_width_for_percent(kDutyCyclePercents[0]),
  pulse_width_for_percent(kDutyCyclePercents[1]),
  pulse_width_for_percent(kDutyCyclePercents[2]),
  pulse_width_for_percent(kDutyCyclePercents[3]),
...
int app::run(void* context) {
  ...
  ret = demo_pwm.configure(kPwmPeriodNs,
                           pulse_width_for_percent(kDutyCyclePercents[0]));
  if (ret < 0) {
    return ret;
  }
  ...
  ret = demo_pwm_sequence.play_pulse_sequence(kPulseSequenceNs,
                                              std::size(kPulseSequenceNs), 0U);
  if (ret < 0) {
    return ret;
  }
